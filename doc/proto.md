...
*length* is used as an effect length will it utilize a full 64-bit
value.
...
- bit 12, `NBD_FLAG_BLOCK_STATUS_PAYLOAD`: Indicates that the server
  ...
  advertise this bit in response to `NBD_OPT_GO` if the client does
  not negotiate metacontexts with `NBD_OPT_SET_META_CONTEXT`; clients
  SHOULD NOT set the `NBD_CMD_FLAG_PAYLOAD_LEN` flag for
  `NBD_CMD_BLOCK_STATUS` unless this transmission flag is set.
